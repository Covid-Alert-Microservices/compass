...
public interface GraphRepository extends Neo4jRepository<UserEntity,String> {
    @Query("MERGE (a:Person {id: {user1}) " +
            "MERGE (b:Person {id: {user2}}) " +
            "MERGE (a)-[:SEE {timestamp: {time}]->(b)")
    void insert(@Param("user1") String user1, @Param("user2") String user2, @Param("time") Long timestamp);

    @Query("MATCH (positive:Person {id: {positiveId}})-[rel:SEE]-(potential:Person) " +
            "WHERE rel.timestamp > {time}" +
            "RETURN potential")
    List<String> findPotentials(@Param("positiveId") String positiveUserId, @Param("time") Long timestamp);

    @Query("MATCH (positive:Person {id: {positiveId}})-[rels:SEE*0..]-(potential:Person) " +
            "WHERE ALL (rel in rels WHERE rel.timestamp > {time}) AND contact.id <> {positiveId} " +
